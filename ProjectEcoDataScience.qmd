...
We test H3 using multivariate regression techniques. In all specifications, the independent variable is the Normalized Distance, as previously defined. In the initial specification, the sales growth is employed as the dependent variable for the measurement of firm performance as by Cucculelli & Bettinelli [-@cucculelli_business_2015]. The second dataset was employed, and all observations with zero values for market value and total assets were removed in order to take the logarithms. Moreover, outliers are only winsorized above the 99th percentile. For a more detailed discussion of this step, see Appendix C. In the second specification the growth of Tobin's Q is employed as described by Desyllas et al. [-@Desyllas2022]. The third dataset is used for this specification. In both specifications, firm size is controlled for using the logarithm of the market value in the first specification and the logarithm of total assets in the second [@dang2018measuring]. In the initial specification we use the interaction term of the year and our BERTScore industry classification as a fixed effect. The interaction term of the year and the industry is employed to capture potential heterogeneous effects of the industry classification across different years. In the second specification we only use industry fixed effects.Furthermore, the time interval between the initial and final years utilized for Tobin's Q growth is controlled for. In the robustness check we use the SIC codes instead of our BERTScore industry classification. The economic and political environment is not controlled for, as White et al. [-@white_exploring_2022] suggest, as all of the companies in question are based in the United States.
