...
- independent variable: ((distance/average distance)-1)*100 = our measure
    - Reason for normalizing: Summaries between years differ on average very little. Difference may be due to small changes in 10-K and   preprocessing with Gemini. Makes the interpretation easy. 
    - Interpretation: If a companies BM changes 1 percentage point more than the market's average, the company has a x percentage points higher revenue growth. 

We control for firm size with the logarithm of the market value. To test the robustness we further use the logarithm of the total assets [@dang2018measuring]. We use fixed effects for the year and our BERTScore industry classification. In the robustness check we use the SIC codes instead of our BERTScore industry classification. We do not control for economical and political environment as White et al. [-@white_exploring_2022] suggests since all of our companies are U.S. based companies.
...
Figure2 <- read.csv("./TableData/Regression/RegressionData.csv")%>%
  ...
  labs(title = "Density Distance", x = "Distance", y = "Density") +
  theme_minimal()
