...
From the dataset containing all the summaries and financials per company per year we construct three datasets for our further analysis. The first dataset is used for the BERTScore industry classification. Therefore, we filter for all summaries of 2017 and for the first summary of companies which do not appear first in 2017 but in a later year. We then calculate the BERTScores between all these summaries. The second and third dataset are used for the regression analysis of our measure. For the first dataset, we fix the company and calculate the BERTScore of the summaries of one year and the consecutive year. 

- based on the data we obtained, which BERT Scores do we calculate and for what do we use them? (between one year and the following -> regression with revenue/sales growth; between companies in one year -> industry classification, between first and last year of appearance in the dataset -> regression Tobin's Q)
